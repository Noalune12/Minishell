...
static int	handle_dollar_sign(char *str, char *expanded, size_t *i, size_t *j)
{
	char	*var_name;
	size_t	var_len;
	...
	if (!var_name)
		return (0);
	copy_var_value(var_name, expanded, j);
	free(var_name);
	*i += var_len - 1;
	return (1);
}
...
char	*expand_env_vars(char *str)
{
	...
	expanded_len = get_expanded_str_len(str);
	...
	while (str && str[i])
	{
		...
		{
			if (!handle_dollar_sign(str, expanded, &i, &j))
			{
				free(expanded);
				return (NULL);
			}
